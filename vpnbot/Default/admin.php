...
⚠️ نام کاربری کانال باید بدون @ ارسال شود", $backadmin, 'HTML');
    ...
    if (!is_array($botSet)) { $botSet = []; }
    if (!isset($botSet['auto_backup_minutes'])) {
        $fallback = select("setting", "*", null, null, "select", ['cache' => false]);
        if (is_array($fallback)) {
            $botSet['auto_backup_minutes'] = intval($fallback['auto_backup_minutes'] ?? 0);
            $botSet['auto_backup_enabled'] = !empty($fallback['auto_backup_enabled']) ? 1 : 0;
        }
    }
...
} elseif ($user['step'] == 'set_backup_resources') {
    ...
    if ($ext === 'sql') {
        ...
        if ($zip->open($tmpName) === TRUE) {
            ...
            if (is_dir($srcData)){
                if (is_dir('data')){
                    $riiDel = new RecursiveIteratorIterator(new RecursiveDirectoryIterator('data', FilesystemIterator::SKIP_DOTS), RecursiveIteratorIterator::CHILD_FIRST);
                    foreach($riiDel as $df){ $df->isDir() ? rmdir($df->getPathname()) : unlink($df->getPathname()); }
                    rmdir('data');
                }
