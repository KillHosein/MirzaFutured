...
خطای دریافتی :  {$errorDescription}";
...
} elseif ($user['step'] == 'restore_backup_wait' && $adminrulecheck['rule'] == "administrator") {
    if (!$document || !$fileid) {
    ...
    if ($ext === 'sql') {
        ...
        if ($zip->open($tmpName) === TRUE) {
            ...
            $zip->close();
            $srcData = $extractDir . '/vpnbot';
            if (is_dir($srcData)){
                $rii = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($srcData, FilesystemIterator::SKIP_DOTS));
                foreach ($rii as $file){
                    $path = $file->getPathname();
                    $rel = substr($path, strlen($srcData));
                    $dest = __DIR__ . '/vpnbot' . $rel;
                    if (!is_dir(dirname($dest))) mkdir(dirname($dest), 0777, true);
                    copy($path, $dest);
                }
            }
            if (is_file($extractDir.'/product.json')) copy($extractDir.'/product.json', __DIR__.'/vpnbot/Default/product.json');
            if (is_file($extractDir.'/product_name.json')) copy($extractDir.'/product_name.json', __DIR__.'/vpnbot/Default/product_name.json');
            foreach (scandir($extractDir) as $f){
                if (strtolower(pathinfo($f, PATHINFO_EXTENSION)) === 'sql'){
                    try{ $pdo->exec(file_get_contents($extractDir.'/'.$f)); }catch(Throwable $e){ }
                }
            }
    ...
    @unlink($tmpName);
    if ($ok){
        sendmessage($from_id, "✅ عملیات بازیابی بکاپ انجام شد.", $setting_panel, 'HTML');
        step('home', $from_id);
    } else {
